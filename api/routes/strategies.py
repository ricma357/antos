    ...
    StrategyInfo(
        ...
        parameters=[
            ...
            StrategyParam(name="prediction_threshold", type="float", default=0.001,
                          description="Minimum expected return to trigger a trade signal. E.g., 0.001 requires a predicted 0.1% return. Prevents overtrading on low-conviction signals. Only used as a fallback when vol_threshold_k is disabled."),
            StrategyParam(name="vol_threshold_k", type="float", default=0.15,
                          description="Volatility-scaled conviction threshold: a trade requires |predicted return| > k × trailing 20-bar realized volatility. Demands proportionally stronger conviction when the market is noisy — cuts churn and fee drag in volatile regimes (2008: −2.5% vs −9.1% fixed). Set to 0 to disable and use the fixed prediction_threshold."),
            StrategyParam(name="strength", type="float", default=0.50,
